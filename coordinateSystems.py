...
class TangentPlaneCartesianSystem:
    ...
    def fromLocal(self, data):
        """Transforms 3xN array of data (position vectors in a tangent plane cartesian system centered at location specified by (ctrLat, ctrLon, ctrAlt) above to the ECEF system
           Returns another 3xN array
        """
        return (array( [dot(inv(self.TransformToLocal), (v)[:,newaxis])for v in data[0:3,:].transpose()]).squeeze().transpose())+self.centerECEF[:,newaxis]
